...
variant<QJsonDocument, QString> RestApi::parseJson(QNetworkReply *reply)
{
    ...
    if (parseError.error == QJsonParseError::NoError && doc.isObject()) {
        ...
        if (obj.contains("error")) {
            const QJsonValue errVal = obj["error"];
            if (errVal.isObject()) {
                const QJsonObject errObj = errVal.toObject();
                QString message = errObj.value("message").toString();
                int status = errObj.value("status").toInt();
                return QString("Spotify API error %1: %2").arg(status).arg(message);
...
QNetworkReply *RestApi::userMusic(int limit) const
{
    return network().get(request("/v1/me/tracks",
